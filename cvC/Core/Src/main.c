...
static volatile uint16_t raw_adc;
...
int main(void)
{
  ...
  while (1)
  {
	  static uint8_t leds[12];
	  static uint8_t *p;
	  p = &leds[2]; // pointer pointing to second position of leds

	  uint16_t speed = (raw_adc * 481 / 4096) + 20;

	  static volatile uint32_t cTime;
	  if (HAL_GetTick() > cTime + speed){
